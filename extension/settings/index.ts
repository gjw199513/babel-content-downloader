...
import { detectLanguage, errorMessageKey, isLanguagePreference, LANGUAGE_KEY, setupInstructions, translate, type Language, type LanguagePreference, type MessageKey } from '../welcome/i18n.js';

type UiState = NonNullable<UiResponse['state']>;

function element<T extends Element>(selector: string): T {
  const result = document.querySelector<T>(selector);
  if (!result) throw new Error(`Missing settings element: ${selector}`);
  return result;
}
...
const setupCopy = element<HTMLElement>('#setup-copy');
const setupClient = element<HTMLSelectElement>('#setup-client');
...
const controls = [...document.querySelectorAll<HTMLButtonElement>('#connect, #pause, #resume, #revoke')];
...
function setControlVisibility(): void {
  const paired = lastState?.paired ?? false;
  const paused = lastState?.paused ?? false;
  const error = currentError ?? lastState?.error;
  const hasConnectionError = !!error && error.code !== 'PAIRING_NEEDED';
  element<HTMLButtonElement>('#connect').hidden = paired && !hasConnectionError;
  element<HTMLButtonElement>('#pause').hidden = !paired || paused || hasConnectionError;
  element<HTMLButtonElement>('#resume').hidden = !paired || !paused || hasConnectionError;
  element<HTMLButtonElement>('#revoke').hidden = !paired;
}
...
function render(): void {
  ...
  for (const node of document.querySelectorAll<HTMLElement>('[data-i18n-label]')) node.setAttribute('aria-label', t(node.dataset.i18nLabel as MessageKey));
  element<HTMLElement>('#copy-setup').textContent = t(copied ? 'copied' : 'copySetup');
  ...
  if (lastState) {
    ...
    setupCopy.textContent = setupInstructions(language, lastState, setupClient.value === 'claude' ? 'claude' : 'codex', navigator.platform);
    const error = currentError ?? lastState.error;
    const hasConnectionError = !!error && error.code !== 'PAIRING_NEEDED';
    ...
    detail.textContent = '';
    setupCopy.textContent = t('loading');
...
async function control(type: Exclude<UiRequest['type'], 'get_state'>): Promise<void> {
  controls.forEach(button => { button.disabled = true; });
  currentError = undefined;
  failureKey = undefined;
  try {
    const response = await chromeMessage(type);
...
setupClient.addEventListener('change', render);
...
for (const type of ['connect', 'pause', 'resume', 'revoke'] as const) element<HTMLButtonElement>(`#${type}`).addEventListener('click', () => { void control(type); });

element<HTMLButtonElement>('#copy-setup').addEventListener('click', () => {
  void navigator.clipboard.writeText(setupCopy.textContent ?? '').then(() => {
    copied = true;
    noticeKey = undefined;
    if (copyTimer) clearTimeout(copyTimer);
    render();
    copyTimer = setTimeout(() => { copied = false; render(); }, 1500);
  }).catch(() => { noticeKey = 'copyFailed'; render(); });
});
